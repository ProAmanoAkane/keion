...
class MusicCog(commands.Cog):
    ...
    async def get_music_info(self, query: str) -> dict:
        ...
        if is_valid_url(query):
            info = await loop.run_in_executor(None, 
                self.downloader.extract_info, query, False)
        else:
            search = await loop.run_in_executor(None,
                self.downloader.extract_info, f"ytsearch1:{query}", False)
    ...
    def play_next(self, context: Context, error: Exception = None) -> None:
        ...
        if self.loop_song and self.current_song:
            asyncio.run_coroutine_threadsafe(
                self.play_song(context, self.current_song), 
                self.bot.loop
            )
            return
        ...
        if self.playlist:
            ...
            asyncio.run_coroutine_threadsafe(
                self.play_song(context, next_song),
                self.bot.loop
            )
            ...
            asyncio.run_coroutine_threadsafe(
                self.voice_clients[context.guild.id].disconnect(),
                self.bot.loop
            )
    ...
    async def play(self, context: Context, *, query: str) -> None:
        ...
        if not self.voice_clients[context.guild.id].is_playing():
            ...
            embed.add_field(name=info["title"], 
    ...
    async def skip(self, context: Context) -> None:
        ...
        if context.guild.id in self.voice_clients and self.voice_clients[context.guild.id].is_playing():
    ...
    async def resume(self, context: Context) -> None:
        ...
        if context.guild.id in self.voice_clients and self.voice_clients[context.guild.id].is_paused():
